...
public final class PairingConnectionCtx implements Closeable {
    ...
    private byte[] exportKeyingMaterial(SSLSocket sslSocket, int length) throws SSLException {
        ...
        try {
            // Approach 1: Direct method on SSLSocket (Android 14+ / newer Conscrypt)
            try {
                Method ekm = sslSocket.getClass().getMethod("exportKeyingMaterial", String.class, byte[].class, int.class);
                return (byte[]) ekm.invoke(sslSocket, EXPORTED_KEY_LABEL, null, length);
            } catch (NoSuchMethodException ignored) {}

            // Approach 2: Conscrypt static method with SSLSocket param (older Android)
            ...
            // Try 3-param version (some versions dropped the context param)
            try {
                Method exportKeyingMaterial = conscryptClass.getMethod("exportKeyingMaterial", SSLSocket.class,
                        String.class, int.class);
                return (byte[]) exportKeyingMaterial.invoke(null, sslSocket, EXPORTED_KEY_LABEL, length);
            } catch (NoSuchMethodException ignored) {}

            // Approach 3: Try via Conscrypt.Sockets inner class
            try {
                Class<?> socketsClass = Class.forName(conscryptClass.getName() + "$Sockets");
                Method ekm = socketsClass.getMethod("exportKeyingMaterial", SSLSocket.class,
                        String.class, byte[].class, int.class);
                return (byte[]) ekm.invoke(null, sslSocket, EXPORTED_KEY_LABEL, null, length);
            } catch (Exception ignored) {}

            throw new SSLException("Could not find exportKeyingMaterial method on this platform (SDK " + Build.VERSION.SDK_INT + ")");
